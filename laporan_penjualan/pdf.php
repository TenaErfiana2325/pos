...
if (!empty($tgl_mulai) && !empty($tgl_selesai)) {
    ...
    $query = mysqli_query($con, "
        ...
        ORDER BY tgl_penjualan DESC
    ") or die(mysqli_error($con));
    $sub_judul = 'Semua Periode Penjualan';
}
...
$pdf->Cell(186, 7, 'LAPORAN PENJUALAN', 0, 1, 'C');
...
$pdf->Cell(56, 8, 'Tgl. Penjualan', 1, 0, 'C', true);
$pdf->Cell(60, 8, 'Total Penjualan', 1, 1, 'C', true);
...
if (mysqli_num_rows($query) > 0) {
    while ($row = mysqli_fetch_array($query)) {
        ...
        $pdf->Cell(56, 7, $tgl_jual, 1, 0, 'C', $fill);
    ...
    $pdf->Cell(186, 8, 'Tidak ada data penjualan pada rentang tanggal ini.', 1, 1, 'C');
}
...
$pdf->Cell(120, 5, '', 0, 0);
$pdf->Cell(66, 5, '( Admin Operasional )', 0, 1, 'C');

// Output PDF langsung ke tab baru browser
$pdf->Output('I', 'Laporan_Penjualan_CinderennaMart.pdf');
exit;
?>