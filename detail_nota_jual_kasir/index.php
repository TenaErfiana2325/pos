...
// Query Header Nota Jual & Supplier menggunakan WHERE dan AND (tanpa JOIN)
$tgl_penjualan = '-';
$total_penjualan = 0;
$keterangan = '-';
$nama_supplier = '-';
...
if (!empty($kode_nota_get)) {
    // Ambil data nota beserta nama supplier-nya
    $q_nota = mysqli_query($con, "
        SELECT n.*, s.nama_supplier 
        FROM tbl_notajual n, tbl_supplier s
        WHERE n.kode_supplier = s.kode_supplier
        AND n.kode_nota = '$kode_nota_get'
    ") or die(mysqli_error($con));
    
    if ($d_nota = mysqli_fetch_array($q_nota)) {
        $tgl_penjualan = $d_nota['tgl_penjualan'] ?? ($d_nota['tgl_nota'] ?? '-');
        $total_penjualan = $d_nota['total_penjualan'] ?? ($d_nota['total_harga'] ?? ($d_nota['grand_total'] ?? 0));
        $keterangan = $d_nota['keterangan'] ?? '-';
        $nama_supplier = $d_nota['nama_supplier'] ?? '-';
    } else {
        // Jika kode_supplier tidak ada di tbl_supplier, tetap ambil data nota
        $q_nota_single = mysqli_query($con, "
            SELECT * FROM tbl_notajual WHERE kode_nota = '$kode_nota_get'
        ") or die(mysqli_error($con));
...
<head>
  ...
  <title>Detail Nota Jual</title>
  <?php 
  include '../css.php';
  $hal = 'detail_nota_beli_kasir';
  ?>
  <style>
  ...
  <div class="content-wrapper">
    ...
    <div class="content">
      <div class="container-fluid">
        ...
        <!-- CARD DETAIL INFORMASI NOTA JUAL -->
        <div class="card card-primary">
          <div class="card-header">
            <h3 class="card-title">Detail Informasi Nota Jual</h3>
          </div>
          <div class="card-body">
            <div class="row">
              <div class="col-md-6">
              ...
              <div class="col-md-6">
                <table class="table table-borderless table-sm">
                  <tr>
                    <td width="35%"><strong>Total Penjualan</strong></td>
                    <td width="5%">:</td>
        ...
        <div class="card card-primary card-tabs">
          ...
          <div class="card-body">
            <div class="mb-3">
              ...
              <a href="../nota_jual_kasir/index.php" class="btn btn-secondary">
            ...
            <table class="table table-bordered table-striped example-table" style="width:100%">
              ...
              if ($tab_active == 'barang') {
                  ...
                  $q_detail = mysqli_query($con, "
                    ...
                    FROM tbl_detail_notajual d, tbl_konsinyasi k
                    WHERE d.kode_brg = k.kode_barang
                    AND d.kode_nota = '$kode_nota_get'
                    AND d.jenis_barang = 'KONSINYASI'
                  ") or die(mysqli_error($con));

              } else {
                  // Tab Semua (Reguler & Konsinyasi)
                  $q_detail = mysqli_query($con, "
                    SELECT d.*, b.nama_brg AS nama_barang
                    FROM tbl_detail_notajual d, tbl_barang b
                    WHERE d.kode_brg = b.kode_brg
              ...
              ?>
                <tr>
                  ...
                  <td>
                    <span class="badge <?= ($data['jenis_barang'] == 'REGULER') ? 'badge-info' : 'badge-warning'; ?>">
                  ...
                  <td>
                    <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modal-edit"
                      ...
                      data-jenis_barang="<?= $data['jenis_barang']; ?>"
        ...
        <form action="tambah.php" method="post">
          <div class="modal-body">
              ...
              <select name="kode_brg" id="tambah_kode_brg" class="form-control" required>
                ...
                <optgroup label="Barang Reguler">
                  ...
                  while ($d_brg = mysqli_fetch_array($q_brg)) {
                    $harga_jual = $d_brg['harga_jual'] ?? 0;
                    echo '<option value="' . $d_brg['kode_brg'] . '" data-harga="' . $harga_jual . '" data-jenis="REGULER">' . $d_brg['kode_brg'] . ' - ' . $d_brg['nama_brg'] . '</option>';
                  }
                  ?>
                </optgroup>
                <optgroup label="Barang Konsinyasi">
                  <?php
                  $q_brg_k = mysqli_query($con, "SELECT * FROM tbl_konsinyasi") or die(mysqli_error($con));
                  while ($d_brg_k = mysqli_fetch_array($q_brg_k)) {
                    $harga_jual = $d_brg_k['harga_jual'] ?? 0;
                    echo '<option value="' . $d_brg_k['kode_barang'] . '" data-harga="' . $harga_jual . '" data-jenis="KONSINYASI">' . $d_brg_k['kode_barang'] . ' - ' . $d_brg_k['nama_barang'] . '</option>';
                  }
                  ?>
                </optgroup>
              </select>
            ...
            <div class="form-group">
              ...
              <input type="number" name="jumlah" class="form-control" id="tambah_jumlah" placeholder="Masukkan Jumlah" required>
            </div>

            <div class="form-group">
              <label for="harga_jual">Harga Jual</label>
              <input type="number" name="harga_jual" class="form-control" id="tambah_harga" placeholder="Masukkan Harga Jual" required>
            </div>

            <div class="form-group">
              <label for="total_harga_beli">Total Harga Beli</label>
              <input type="number" name="total_harga_beli" class="form-control" id="tambah_total" placeholder="Masukkan Total Harga" required>
            </div>
          </div>
          ...
          <div class="modal-body">
            ...
            <div class="form-group">
              ...
              <select name="kode_brg" id="edit_kode_brg" class="form-control" required>
                ...
                <optgroup label="Barang Konsinyasi">
                  <?php
                  $q_brg_edit_k = mysqli_query($con, "SELECT * FROM tbl_konsinyasi") or die(mysqli_error($con));
            ...
            <div class="form-group">
              <label for="jumlah">Jumlah</label>
              <input type="number" name="jumlah" id="edit_jumlah" class="form-control" placeholder="Masukkan Jumlah" required>
            </div>

            <div class="form-group">
              <label for="harga_jual">Harga Jual</label>
              <input type="number" name="harga_jual" id="edit_harga" class="form-control" placeholder="Harga Jual" required>
            </div>
...
<script>
  $(document).ready(function() {
    ...
    // Mengisi Modal Edit secara dinamis
    $('#modal-edit').on('show.bs.modal', function(e) {
      ...
      $('#edit_harga').val(button.data('harga_jual'));
