        ...
        <div class="card">
              ...
              <div class="card-body">
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                  <?php 
                  $panggil_data_barang = mysqli_query($con, "SELECT * FROM tbl_barang") or die(mysqli_error($con));
                  ...
                  if ($rv > 0) {
                    while ($data = mysqli_fetch_array($panggil_data_barang)) {
                      ...
                      <tr>
                        ...
                        <td><?= $data['merk'] ?></td>
                        <td><?= $data['stok'] ?></td>
                        <td>Rp <?= number_format($data['rata_harga_beli'], 0, ',', '.') ?></td>
                        <td>Rp <?= number_format($data['harga_jual'], 0, ',', '.') ?></td>
                        <td>
                          <svg class="barcode" 
                              jsbarcode-value="<?= !empty($data['barcode']) ? $data['barcode'] : $data['kode_brg']; ?>"
                              jsbarcode-height="30"
                        ...
                        <?php if (!empty($data['foto_brg']) && file_exists('img/' . $data['foto_brg'])) : ?>
                          <img src="img/<?= $data['foto_brg']; ?>" width="70" class="img-thumbnail" alt="<?= $data['nama_brg']; ?>">
                        <?php else : ?>
                          <span class="badge bg-secondary">Tidak ada foto</span>
                        <?php endif; ?>
                      </td>
                        <td>
                          <a href="hapus.php?kode=<?= urlencode($data['kode_brg']); ?>" 
                            class="btn btn-danger btn-sm" 
                            onclick="return confirm('Yakin ingin menghapus data barang ini?')">
                            <i class="fas fa-trash"></i>
                          </a>
                          <a href="edit.php?kode=<?= urlencode($data['kode_brg']); ?>" 
        ...
        <form action="tambah.php" method="post" enctype="multipart/form-data">
          <div class="modal-body">
            ...
            <div class="form-group">
              ...
              <input type="text" name="merk" class="form-control" id="merk" placeholder="Masukkan Merk Barang" required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <input type="number" name="rata_harga_beli" class="form-control" id="rata_harga_beli" placeholder="Masukkan Harga Beli" required>
            </div>
            <div class="form-group">
